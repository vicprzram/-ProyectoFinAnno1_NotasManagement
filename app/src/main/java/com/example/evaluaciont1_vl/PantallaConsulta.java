...
public class PantallaConsulta extends AppCompatActivity implements View.OnClickListener {
    private static final CharSequence BTN_LIMPIAR = "Limpiar datos";
    private static final CharSequence BTN_SELECCION ="Seleccionar alumno" ;
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data){
        ...
        if(requestCode == 1) {
            if (resultCode == RESULT_OK) {
                ...
                Toast.makeText(this,R.string.no_seleccionado , Toast.LENGTH_LONG).show();
                limpiarDatos();
...
//TODO: HAY QUE ACABAR CON MOSTRAR NOTAS NO PUEDO COMPROBAR PORQUE
// "ERROR: No se ha encontrado el archivo: /data/data/com.example.evaluaciont1_vl/files/database/database.dat"
    ...
    private void agregarFragmento(NotaFragmento nota) {//agregamos fragmento dinamico al LinearLayout
        FragmentManager fm = getSupportFragmentManager();
        FragmentTransaction ft = fm.beginTransaction();
        NotaFragmento nf = new NotaFragmento();

        ft.replace(fl.getId(), nf);
